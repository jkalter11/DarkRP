...
local playerName = ""
RegisterNetEvent("darkrp:f_getName")
AddEventHandler("darkrp:f_getName", function(name)
  playerName = name
end)

local newBalance = 0
RegisterNetEvent("darkrp:f_getBalance")
AddEventHandler("darkrp:f_getBalance", function(bal)
  newBalance = bal
end)

-- Open Gui and Focus NUI
function openGui()
  TriggerServerEvent("darkrp:getName")
  TriggerServerEvent("darkrp:getBalance")
  Wait(200)
  SetNuiFocus(true)
...
RegisterNetEvent('darkrp:deposit')
AddEventHandler('darkrp:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent("darkrp:notify", "~r~ Vous ne pouvez pas utiliser d'atm en véhicule")
    else
      TriggerServerEvent("darkrp:deposit", tonumber(amount))
    end
  else
    TriggerEvent("darkrp:notify", "~r~ Vous ne pouvez déposer que dans une banque")
  end
end)

-- Process withdraw if conditions met
RegisterNetEvent('darkrp:withdraw')
AddEventHandler('darkrp:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
      TriggerEvent("darkrp:notify", "~r~ Vous ne pouvez pas utiliser d'atm en véhicule")
    else
      TriggerServerEvent("darkrp:withdraw", tonumber(amount))
    end
  else
    TriggerEvent("darkrp:notify", "~r~ Ce n'est pas une banque ni un ATM")
  end
end)

-- Process give cash if conditions met
RegisterNetEvent('darkrp:givecash')
AddEventHandler('darkrp:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      TriggerServerEvent("darkrp:givecash", toPlayer, tonumber(amount))
    else
      TriggerEvent("darkrp:notify", "~r~ Le joueur n'est pas en ligne")
    end
  else
    TriggerEvent("darkrp:notify", "~r~ Le joueur doit être proche de vous")
  end
end)

-- Process bank transfer if player is online
RegisterNetEvent('darkrp:transfer')
AddEventHandler('darkrp:transfer', function(fromPlayer, toPlayer, amount)
  local player2 = GetPlayerFromServerId(toPlayer)
  local playing = IsPlayerPlaying(player2)
  if (playing ~= false) then
    TriggerServerEvent("darkrp:transfer", fromPlayer, toPlayer, tonumber(amount))
  else
    TriggerEvent("darkrp:notify", "~r~ Le joueur n'est pas en ligne")
  end
end)
...
RegisterNetEvent('darkrp:updateBalance')
AddEventHandler('darkrp:updateBalance', function(balance)
  TriggerServerEvent("darkrp:getName")
  Wait(200)
	SendNUIMessage({
		updateBalance = true,
...
RegisterNetEvent("darkrp:addBalance")
AddEventHandler("darkrp:addBalance", function(amount)
	SendNUIMessage({
		addBalance = true,
		amount = amount
	})
...
RegisterNetEvent("darkrp:removeBalance")
AddEventHandler("darkrp:removeBalance", function(amount)
	SendNUIMessage({
		removeBalance = true,
		amount = amount
	})
...
RegisterNetEvent('darkrp:updatedBalance')
AddEventHandler('darkrp:updatedBalance', function(balance)
  TriggerServerEvent("darkrp:getName")
  Wait(200)
  SendNUIMessage({
    updatedBalance = true,
...
-- Send NUI Message to display add balance popup
RegisterNetEvent("darkrp:adddBalance")
AddEventHandler("darkrp:adddBalance", function(amount)
  SendNUIMessage({
    adddBalance = true,
    damount = amount
  })
...
-- Send NUI Message to display remove balance popup
RegisterNetEvent("darkrp:removedBalance")
AddEventHandler("darkrp:removedBalance", function(amount)
  SendNUIMessage({
    removedBalance = true,
    damount = amount
  })
...
RegisterNetEvent("darkrp:setBankDisplay")
AddEventHandler("darkrp:setBankDisplay", function(val)
	SendNUIMessage({
		setDisplay = true,
		display = val
	})
