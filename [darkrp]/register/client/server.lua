...
RegisterServerEvent('darkrp:getHair')
AddEventHandler('darkrp:getHair', function(hair, hairsec, haircolor, haircolorsec)
...
RegisterServerEvent('darkrp:getFace')
AddEventHandler('darkrp:getFace', function(e, v, t)
  TriggerEvent('darkrp:getPlayerFromId', source, function(user)
    if (user) then
      local player = user.identifier
      local sexe = "mp_m_freemode_01"
...
RegisterServerEvent('darkrp:getOutfit')
AddEventHandler('darkrp:getOutfit', function(e)
  TriggerEvent('darkrp:getPlayerFromId', source, function(user)
    if (user) then
      local player = user.identifier
...
RegisterServerEvent('darkrp:lastChar')
AddEventHandler('darkrp:lastChar', function()
  TriggerEvent('darkrp:getPlayerFromId', source, function(user)
    if (user) then
      local player = user.identifier
      ...
      MySQL.Async.fetchAll("SELECT skin,face,face_text,hair,hair_text,pants,pants_text,shoes,shoes_text,torso,torso_text,shirt,shirt_text,three,three_text,seven,seven_text,haircolor,haircolor_text FROM outfits WHERE identifier=@user",{
        ...
        TriggerClientEvent("darkrp:updateChar",source,{result[1].face,result[1].face_text,result[1].hair,result[1].hair_text,result[1].pants,result[1].pants_text,result[1].shoes,result[1].shoes_text,result[1].torso,result[1].torso_text,result[1].shirt,result[1].shirt_text,result[1].three,result[1].three_text,result[1].seven,result[1].seven_text,result[1].haircolor,result[1].haircolor_text,result[1].skin})
      end)
    else
      TriggerEvent("darkrp:desyncMsg")
    end
...
RegisterServerEvent('darkrp:lastCharInShop')
AddEventHandler('darkrp:lastCharInShop', function(model)
  TriggerEvent('darkrp:getPlayerFromId', source, function(user)
    if (user) then
      local player = user.identifier
      ...
      MySQL.Async.fetchAll("SELECT skin,face,face_text,hair,hair_text,pants,pants_text,shoes,shoes_text,torso,torso_text,shirt,shirt_text,three,three_text,seven,seven_text,haircolor,haircolor_text FROM outfits WHERE identifier=@user",{['@user']=player}, function (result)
        TriggerClientEvent("darkrp:updateCharInShop",source,{result[1].face,result[1].face_text,result[1].hair,result[1].hair_text,result[1].pants,result[1].pants_text,result[1].shoes,result[1].shoes_text,result[1].torso,result[1].torso_text,result[1].shirt,result[1].shirt_text,result[1].three,result[1].three_text,result[1].seven,result[1].seven_text,result[1].haircolor,result[1].haircolor_text,model})
      end)
    else
      TriggerEvent("darkrp:desyncMsg")
    end
...
RegisterServerEvent('darkrp:fromSlastChar')
AddEventHandler('darkrp:fromSlastChar', function(source)
  TriggerEvent('darkrp:getPlayerFromId', source, function(user)
    if (user) then
      local player = user.identifier
